...
def inputScreen(file_index,files,path):
    """
    
    file_index : an int
    
    selects a minimal pair
    TODO: everything

    """
    wordTuple = ("bat","pat")
    minimalPair = {"left":"bat","right":"pat"}
    global drawn
    global ID
    endoftrial = False
    complete = False
    exitWindow = False
    recordDescriptor="select word\n<- ->"
    wordSelection = "["+minimalPair["left"]+"]   ["+minimalPair["right"]+"]"
    ...
    while not complete:
        ...
        # descriptor text & word selection
        txt.textWrap(screenDisplay, recordDescriptor, bodyText, pygame.Rect((40,40,p.screen_width, p.recBarWidth)), p.OFFWHITE, p.BG, 1) 
        txt.textWrap(screenDisplay, wordSelection, bodyText, pygame.Rect((40,40,p.screen_width, p.recBarWidth)), p.OFFWHITE, p.BG, 1) 
        ...
        for event in events :
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LEFT:
                    appendToAnswerSheet("leftSelection")
                    complete = True
                if event.key == pygame.K_RIGHT:
                    appendToAnswerSheet("rightSelection")
